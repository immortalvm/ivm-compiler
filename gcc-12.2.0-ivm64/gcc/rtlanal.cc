...
for_each_inc_dec_find_inc_dec (rtx mem, for_each_inc_dec_fn fn, void *data)
{
  ...
  switch (GET_CODE (x))
	...
	poly_int64 size = GET_MODE_SIZE (GET_MODE (mem));
#else
    // IVM64: use the pointer size, not of the memory location
    //--
    //--  Test with this simplified version of gcc testsuite
    //--  test gcc.dg-struct-layout-1/t024_*.c:
    //--
    //--        typedef _Complex float CF ;
    //--        struct S { struct{ CF c;} a[3]; } ;
    //--        struct S ss;
    //--        long fails=0;
    //--
    //--        __attribute__((noinline, noipa))
    //--        void check (struct S arg) {
    //--            if (arg.a[2].c != ss.a[2].c)
    //--                 ++fails;
    //--        }
    //--
    //--        int main(){
    //--            ss.a[2].c = 12345 + 1.0iF*3.14;
    //--            check(ss);
    //--            return fails;
    //--        }
    //--
	poly_int64 size = GET_MODE_SIZE (GET_MODE (x));
#endif
	rtx r1 = XEXP (x, 0);
	rtx c = gen_int_mode (-size, GET_MODE (r1));
	return fn (mem, x, r1, r1, c, data);
