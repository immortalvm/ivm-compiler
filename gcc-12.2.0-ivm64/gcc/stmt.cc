  ...
  if (dispatch_table.length () <= 5
      ...
      || !flag_jump_tables)
    {
	 ...
	 and will be cleaned up by later passes.  */
	  ...
	  #ifdef TARGET_IVM64
	  if (!inc1_ok){
			// If force_expand_binop with OPTAB_DIRECT fails, 
            // try other methods (remember there are not
            // native arithmetic patterns in ivm64.md for
            // SImode, only DImode)
			// Motivated by the dispatch table expansion
            // in nested try/catch, when catch branch calls a throw
            // If this function fails silently and the index
            // is not decreasing to traverse all the possibities
            // after returning from longjump, givin rise to an abort
	  		force_expand_binop (index_mode, sub_optab,
			      index, CONST1_RTX (index_mode),
			      index, 0, OPTAB_WIDEN);
	  }
