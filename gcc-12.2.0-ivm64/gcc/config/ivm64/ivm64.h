...
#define IVM64_GCC_VERSION "3.4.1"
...
/* The rdynamic flag is motivated by cmake, that, in linux, checks
   the compiler with -rdynamic. Actually the option does nothing. */
#define LINK_SPEC "%{rdynamic:-ignore}"
...
#define ASM_GENERATE_INTERNAL_LABEL(LABEL,PREFIX,NUM) \
 sprintf(LABEL, "*.%s%ld", PREFIX, (unsigned long)(NUM))
...
#define ASM_FORMAT_PRIVATE_NAME(OUTPUT, NAME, LABELNO) \
 ((OUTPUT) = (char *) alloca (strlen ((NAME)) + 31),   \
   sprintf((OUTPUT), ".Li_%s_%lu", (NAME), (unsigned long)(LABELNO)))
...
do { \
     char *buf = (char *)alloca(strlen(PREFIX)+31); \
...
#define JMP_BUF_SIZE  ((3+2+16)*sizeof(unsigned long))

/* This hook defines the mechanism that will be used for exception handling by the
   target. If the target has ABI specified unwind tables, the hook should return UI_
   TARGET. If the target is to use the setjmp/longjmp-based exception handling scheme,
   the hook should return UI_SJLJ. If the target supports DWARF 2 frame unwind
   information, the hook should return UI_DWARF2.
   A target may, if exceptions are disabled, choose to return UI_NONE. This may end
   up simplifying other parts of target-specific code. The default implementation of this
   hook never returns UI_NONE.

   The default implementation of the hook first honors the ‘--enable-sjlj-exceptions’
   configure option, then DWARF2_UNWIND_INFO, and finally defaults to UI_SJLJ. If
   DWARF2_UNWIND_INFO depends on command-line options, the target must define this
   hook so that opts is used correctly. */
//#define TARGET_EXCEPT_UNWIND_INFO UI_SJLJ

/*
#define EH_RETURN_DATA_REGNO(N) (((N) < 3) ? ((N) + 12) : INVALID_REGNUM)
#define EH_RETURN_HANDLER_RTX \
  gen_rtx_MEM (Pmode, gen_rtx_PLUS (Pmode, gen_rtx_REG (Pmode, STACK_POINTER_REGNUM), \
                  gen_rtx_REG (Pmode, 15)))
#define EH_RETURN_STACKADJ_RTX gen_rtx_REG (Pmode, 15)
#define ASM_PREFERRED_EH_DATA_FORMAT(CODE,GLOBAL) DW_EH_PE_udata8
*/
...
#define RETURN_ADDR_RTX(COUNT,FP) (ivm64_return_addr(COUNT, FP))
...
enum ivm64_peephole2 {
 ...
 IVM64_PEEP1_PUSH_POPAR                     ,
 IVM64_PEEP1_PUSH_POPMEMAR
};
