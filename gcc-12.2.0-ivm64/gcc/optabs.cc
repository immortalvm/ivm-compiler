	      ...
	      rtx target, int unsignedp, enum optab_methods methods)
{
  ...
  //-- in nested try/catch, when catch branch throw an exception.
  //-- See stmt.c:expand_sjlj_dispatch_table().
  //-- This function fails silently for modes smaller
  //-- than DI and methods=OPTAB_DIRECT
  int ivm64_mode_size;
  if ( is_int_mode (mode, &ivm64_mode_size)
       && ivm64_mode_size < UNITS_PER_WORD 
