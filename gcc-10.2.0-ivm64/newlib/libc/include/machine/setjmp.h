...
#ifdef __ivm64__
/*
   To be compatible with gcc __builtin_setjmp(), the buffer has to have
   five words at least (see gimple-low.c:725 lower_builtin_setjmp()).
   This positions of the buffer are used to save the following registers: 
        0: fp
        1: pc
        2: sp
        3: not used 
        4: not used 
    In addition we add extra positions to potentially store the 
    general purpose stack-mapped registers or any other extra information
*/
/*   test with 'testsuite/gcc.dg/setjmp-[34].c' */
#define _JBLEN 1
#define _JBTYPE  struct {unsigned long fp; unsigned long pcret; unsigned long sp; \
                         unsigned long unused[2]; unsigned long extra[16];} 
//#define _JBTYPE  struct {unsigned long sp; unsigned long pcret; unsigned long extra[8];} 
#endif
