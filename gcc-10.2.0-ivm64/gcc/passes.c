					...
					vec<uid_range_p> tab)
{
  ...
  #else
  // There is a bug due to which if a pass get disable explicitly
  // with -fdisable-<pass_name>, gcc crashes with the option -fdump-passess 
  // as null pointer for cgraph_node::get is not checked
      cgraph_uid = !func ? 0 : cgraph_node::get (func) ? cgraph_node::get(func)->get_uid() : 0;
  #endif
