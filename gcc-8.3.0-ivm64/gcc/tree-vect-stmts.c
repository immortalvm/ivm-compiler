...
/*e Calling function multiple_of_p(...) with 4 arguments caused errors
      So, we emulated it here (see wide-int.h:2934 for its definition)
  */
/* if (!wi::multiple_of_p (wi::to_widest (step), scale, SIGNED, &factor))
      	continue;
*/
      widest_int x_e = wi::to_widest (step);  
      widest_int y_e = scale;  
      signop sgn_e = SIGNED;
      int is_multiple_of_p_e = wi::multiple_of_p (x_e, y_e, sgn_e);
      if (is_multiple_of_p_e)
      {
        widest_int remainder;
        factor = wi::divmod_trunc (x_e, y_e, sgn_e, &remainder);
      }
      if (!is_multiple_of_p_e)
        continue;
